defmodule Ginga.EventManager do
	@moduledoc """
Ginga's application events manager.
"""
	...
	def notify_user_added(user) do
		Logger.debug "Notifying: user #{user.first_name} was added"
  ...
  def notify_event_report_added(event_report) do
	...
	defp register_handlers() do
		...
		GenEvent.add_handler(@name, Ginga.InternalEventHandler, [])
		GenEvent.add_handler(@name, Ginga.UserRegistrationEvent, [])
