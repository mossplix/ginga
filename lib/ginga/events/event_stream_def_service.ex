defmodule Ginga.EventStreamDefService do
    require Amnesia
    use Gingadb
	@moduledoc """
  Ginga's event stream definition service.
  """
  ...
  def get_all_by_user(user) do
			EventStreamDef.get_all(user_id: user.email)
  end
	...
	def user_event_stream_def(user) do
		filter_def = FilterDef.source(user.email)
		%EventStreamDef{description: "All events from #{user.first_name} (#{user.email})", user_id: user.email, shared: true, sub_stream_defs: [], filters: [filter_def]}
	end
  ...
  def properties_event_stream_def([user: user, filter_options: filter_options]) do
		...
		%EventStreamDef{description: "Ad hoc event stream", user_id: user.email, shared: false, sub_stream_defs: [], filters: [filter_def]}
  end
	...
	def visible?(event_stream_def, user) do
		event_stream_def.shared or event_stream_def.user_id === user.email
	end
